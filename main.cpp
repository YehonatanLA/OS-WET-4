...
public:
    ...
    Statistics(size_t a, size_t b, size_t c, size_t d, size_t e, size_t f);
...
Statistics::Statistics(size_t a, size_t b, size_t c, size_t d, size_t e, size_t f) {
    num_free_blocks = a;
    num_free_bytes = b;
...
void Statistics::setAndPrint() {
    set();
    print();
}

bool Statistics::compare(size_t expected_num_free_blocks, size_t expected_num_free_bytes,
                         size_t expected_num_allocated_blocks,
                         size_t expected_num_allocated_bytes, size_t expected_num_meta_bytes,
                         size_t expected_size_meta_data, int line) {
...
// bool Statistics::compare(size_t expected_num_free_blocks, size_t expected_num_free_bytes, size_t expected_num_allocated_blocks, 
//                     size_t expected_num_allocated_bytes, size_t expected_num_meta_bytes, size_t expected_size_meta_data)
// {
...
void test_challenge_1_2() {   //comments represents actual memory
...
void test_challenge_1_3() {   //comments represents actual memory
...
void test_challenge_1_4() {   //comments represents actual memory
...
void test_challenge_2() {   //comments represents sorted list
    current_test_num = 2;
    // cout << "running test_challenge " << current_test_num << "...";
...
void test_challenge_3_1() {   //comments represents actual memory
...
void test_srealloc_common_sizes_a() {   //comments represents actual memory
    //current_test_num = 3.1;
    ...
    sfree(p8);
    // printMemory();
    void *p12 = srealloc(p9,
                         2000); // a - no merging, split after useing the block. (should merge 2 free block afterwards?)
    // printMemory();
...
void test_srealloc_common_sizes_b1() {   //comments represents actual memory
...
void test_srealloc_common_sizes_b2() {   //comments represents actual memory
    ...
    sfree(p9);
    void *p11 = srealloc(p10,
                         6000); // b, p9 is wilderness - merge with lower address, and dont enlarge - need to split afterwards
    s.set();
    s.compare(1, 4000, 2, 10000, 2 * MD_SIZE, MD_SIZE, __LINE__);
}

void test_srealloc_common_sizes_c() {   //comments represents actual memory
...
void test_srealloc_common_sizes_d() {   //comments represents actual memory
...
void test_srealloc_common_sizes_e() {   //comments represents actual memory
...
void test_srealloc_common_sizes_e2() {   //comments represents actual memory
    ...
    sfree(p3);
    void *p4 = srealloc(p2,
                        13000); // e, lower and higher adjacent blocks, join them all, and dont split the free space to a second block
    s.set();
    s.compare(0, 0, 1, 15000 + 2 * MD_SIZE, 1 * MD_SIZE, MD_SIZE, __LINE__);
}

void test_srealloc_common_sizes_f_i() {   //comments represents actual memory
    ...
    sfree(p3);
    void *p4 = srealloc(p2,
                        20000); // f, lower and higher adjacent blocks, cant join them all (not enough space), higer is wilderness - so enlarge
    s.set();
    s.compare(0, 0, 1, 20000, 1 * MD_SIZE, MD_SIZE, __LINE__);
}

void test_srealloc_common_sizes_f_ii() {   //comments represents actual memory
    ...
    void *p4 = srealloc(p2,
                        20000); // f, only higher adjacent block, cant join them (not enough space), higer is wilderness, enlarge
    // p0 p1 p4(2)
    s.set();
    s.compare(0, 0, 3, 30000, 3 * MD_SIZE, MD_SIZE, __LINE__);
...
void test_srealloc_common_sizes_g() {   //comments represents actual memory
    ...
    s.set();
    s.compare(1, 15000 + 2 * MD_SIZE, 5, 5000 + 30000 + 2 * 20000 + 15000 + 2 * MD_SIZE, 5 * MD_SIZE, MD_SIZE,
              __LINE__);
    ...
    s.set();
    s.compare(1, 20000 + 3 * MD_SIZE, 4, 50000 + 2 * 20000 + 15000 + 5000 + 3 * MD_SIZE, 4 * MD_SIZE, MD_SIZE,
              __LINE__);
